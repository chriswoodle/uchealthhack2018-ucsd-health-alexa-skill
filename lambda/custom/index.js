...
const NewAppointmentHandler = {
  canHandle(handlerInput) {
    ...
    return request.type === 'LaunchRequest' || (request.type === 'IntentRequest' && request.intent.name === 'NewAppointment');
  },
  handle(handlerInput) {
    return new Promise(resolve => {
      const requestAttributes = handlerInput.attributesManager.getRequestAttributes();
      const randomFact = 'rrrrggg';
      clinics.getClinics().then(clinics => {
        console.log(clinics[0].name);
        const clinic = 'UC San Diego Health urgent care clinic'; // clinics[0].name
        const message = `Saving your spot at ${clinic}`;
        sms.sendMessage(message);
        return resolve(handlerInput.responseBuilder
          .speak(message)
          .withSimpleCard(requestAttributes.t('SKILL_NAME'), message)
          .getResponse()
        );
      })
    })
...
const GetNewFactHandler = {
  ...
  handle(handlerInput) {
    ...
    const speakOutput = requestAttributes.t('GET_FACT_MESSAGE') + 'RRRRRRGGGGG';
